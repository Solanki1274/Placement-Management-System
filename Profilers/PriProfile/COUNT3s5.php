...
if (isset($_POST['s5'])) { 
    // Sanitize user input
    $Csslc = mysqli_real_escape_string($connection, $_POST['csslc']);

    // Query to count the number of students who scored above the specified SSLC percentage
    $result = mysqli_query($connection, "SELECT COUNT(*) AS total_students FROM basicdetails WHERE `Approve`='1' AND SSLC >= '$Csslc'");
    
    if ($result) {
        $data = mysqli_fetch_assoc($result);
        echo "<br><h3>Students Scored Above '$Csslc' in SSLC&nbsp:&nbsp" . $data['total_students'] . "</h3>"; 
    } else {
        echo "Error counting students: " . mysqli_error($connection);
    }

    // Query to get the details of students who scored above the specified SSLC percentage
    $sql = mysqli_query($connection, "SELECT * FROM basicdetails WHERE `Approve`='1' AND SSLC >= '$Csslc'");
